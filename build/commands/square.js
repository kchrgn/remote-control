...
export const square = (length) => __awaiter(void 0, void 0, void 0, function* () {
    const trajectory = [];
    ...
    for (let dx = 0; dx <= length; dx++) {
        trajectory.push(new Point(x + dx, y - length));
    }
    ;
    for (let dy = 0; dy <= length; dy++) {
        trajectory.push(new Point(x + length, y - length + dy));
    }
    ;
    for (let dx = 0; dx <= length; dx++) {
        trajectory.push(new Point(x + length - dx, y));
    }
    ;
